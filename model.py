...
class DAGMMArrhythmia(DAGMM):
    def __init__(self):
        compressor = CompressionNetworkArrhythmia().cuda()
        estimator = EstimationNetworkArrhythmia().cuda()
        # TODO: the GMM module should be part of the estimator....?
        gmm = GMM(num_mixtures=3, dimension_embedding=4)
...
def relative_euclidean_distance(x1, x2, eps=eps):
    ...
    return num / torch.max(denom, eps.cuda())
